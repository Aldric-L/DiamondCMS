...
if (isset($param[1]) && !empty($param[1]) && isset($param[2])){
    ...
    if (defined("DMcProfileImg") && DMcProfileImg == true){
        ...
        if ($param[1] == "Utilisateur inconnu"){
            ...
            if (!empty($user) && isset($user['profile_img'])){
                if (strpos($user['profile_img'], "png") !== false) {
                    header('Location: ' . $Serveur_Config['protocol'] . '://' . $_SERVER['HTTP_HOST'] . WEBROOT . 'getimage/png/' . substr($user['profile_img'], 0, -4) . '/' . $param[2] . '/'. $param[3]);
                }else if (strpos($user['profile_img'], "jpg") !== false) {
                    header('Location: ' . $Serveur_Config['protocol'] . '://' . $_SERVER['HTTP_HOST'] . WEBROOT . 'getimage/jpg/' . substr($user['profile_img'], 0, -4) . '/' . $param[2] . '/'. $param[3]);
                }else if (strpos($user['profile_img'], "jpeg") !== false) { 
                    header('Location: ' . $Serveur_Config['protocol'] . '://' . $_SERVER['HTTP_HOST'] . WEBROOT . 'getimage/jpeg/' . substr($user['profile_img'], 0, -5) . '/' . $param[2] . '/'. $param[3]);
                }else {
        ...
        if ($param[1] == "Utilisateur inconnu"){
            ...
            if (!empty($user) && isset($user['profile_img'])){
                if (strpos($user['profile_img'], "png") !== false) {
                    header('Location: ' . $Serveur_Config['protocol'] . '://' . $_SERVER['HTTP_HOST'] . WEBROOT . 'getimage/png/' . substr($user['profile_img'], 0, -4) . '/' . $param[2] . '/'. $param[2]);
                }else if (strpos($user['profile_img'], "jpg") !== false) {
                    header('Location: ' . $Serveur_Config['protocol'] . '://' . $_SERVER['HTTP_HOST'] . WEBROOT . 'getimage/jpg/' . substr($user['profile_img'], 0, -4) . '/' . $param[2] . '/'. $param[2]);
                }else if (strpos($user['profile_img'], "jpeg") !== false) { 
                    header('Location: ' . $Serveur_Config['protocol'] . '://' . $_SERVER['HTTP_HOST'] . WEBROOT . 'getimage/jpeg/' . substr($user['profile_img'], 0, -5) . '/' . $param[2] . '/'. $param[2]);
                }else {
                    $controleur_def->addError(130);
                    die('Erreur');
                }
