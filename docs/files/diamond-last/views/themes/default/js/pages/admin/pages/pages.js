        ...
        for (var i = 0; i < new_list.length; i++){
          if (new_list[i].className != "no_pages" && new_list[i].dataset.id != undefined){
            $.ajax({
              ...
              success: function (data_rep) {
                if (data_rep != "Success"){
                  alert("Erreur, Code 112, Merci de contacter les administrateurs du site (Impossible de sauvegarder le nouvel ordre). Réponse du serveur: " + data_rep);
                }
              },
              error: function() {
      ...
      $("#renamer").change(function(){
        ...
        if (value != " " && value != ""){
          $.ajax({
            ...
            success: function (data_rep) {
              if (data_rep != "Success"){
                alert("Erreur code 112 : Impossible de renommer l'élement demandé. Contacter un administrateur. Réponse du serveur: " + data_rep);
                console.log(data_rep);
              }else {    
