...
<div class="content-container-forum">
  ...
  <table class="table table-forum">
    <tr>
      ...
      <div 
          ...
          class="content-post-span"  data-id="<?php echo $post['id']; ?>" data-link="<?php echo $Serveur_Config['protocol']; ?>://<?= $_SERVER['HTTP_HOST']; ?><?=WEBROOT; ?>forum/com/edit_post/<?php echo $post['id']; ?>/"
          <?php } ?>
        >
      ...
      if (isset($_SESSION['admin']) && $_SESSION['admin']) {?>
        ...
        <select class="form-control" style="width: 30%; margin-left:100%; margin-bottom: 10px; margin-top: 10px; float: right; display: none" data-link="<?php echo $Serveur_Config['protocol']; ?>://<?php echo $_SERVER['HTTP_HOST'] . WEBROOT . 'forum/moove/' . $post['id']; ?>" id="cats_available">
          <?php foreach ($scats as $cat){ ?>
                <option value="<?= $cat['id']; ?>" <?php if ($cat['id'] == $post['id_scat']){ ?> selected <?php } ?>><?= $cat['titre'];?></option>
          <?php } ?>
        </select>
        <a class="text-info bold moove" data-id="<?= $post['id']; ?>" data-link="<?php echo $Serveur_Config['protocol']; ?>://<?php echo $_SERVER['HTTP_HOST'] . WEBROOT . 'forum/moove/' . $post['id']; ?>"><i class="fa fa-arrow-circle-o-right" aria-hidden="true"></i> Déplacer le sujet</a>
        
        </p>
        <span class="bold" style="float: right; color: red;"><a class="bold" style="color: red;" href="<?php echo $Serveur_Config['protocol']; ?>://<?php echo $_SERVER['HTTP_HOST'] . WEBROOT . 'forum/del/' . $post['id']; ?>"><i class="fa fa-trash-o" aria-hidden="true"></i> Supprimer le sujet </a></span>
      <?php } ?>
      </td>
    </tr>
  </table>
  ...
  <?php $i =1; foreach ($coms as $key => $com) {?>
    ...
    <table class="table table-forum-com <?php echo $i; ?>">
      <tr>
        ...
        <div 
          ...
          class="content-com-span"  data-id="<?php echo $com['id']; ?>" data-link="<?php echo $Serveur_Config['protocol']; ?>://<?= $_SERVER['HTTP_HOST']; ?><?=WEBROOT; ?>forum/com/edit/<?php echo $com['id']; ?>/"
          <?php } ?>
    ...
    $("#dl_<?php echo $com['id']; ?>").click(function(e){
      e.preventDefault();
      $.get( "<?php echo $Serveur_Config['protocol']; ?>://<?php echo $_SERVER['HTTP_HOST'] . WEBROOT . 'forum/del/com/' . $com['id']; ?>", function( data ) {
        location.reload();
      });
    });
    </script>
