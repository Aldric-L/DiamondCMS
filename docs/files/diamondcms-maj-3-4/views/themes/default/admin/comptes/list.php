...
<div id="modify_modal_<?= $c['id']; ?>" class="modal fade">
  <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-default close_modify_mod" data-link="<?php echo $Serveur_Config['protocol']; ?>://<?= $_SERVER['HTTP_HOST']; ?><?=WEBROOT; ?>admin/comptes/list/ban/" data="<?= $c['id']; ?>">Fermer</button>
          <button type="button" class="btn btn-success mod_button" data-link="<?php echo $Serveur_Config['protocol']; ?>://<?= $_SERVER['HTTP_HOST']; ?><?=WEBROOT; ?>admin/comptes/list/mod/" data="<?= $c['id']; ?>">Enregistrer</button>
          <?php if ($c['profile_img'] != "profiles/no_profile.png"){ ?>
               <button type="button" class="btn btn-warning supp_profile_img" data-link="<?php echo $Serveur_Config['protocol']; ?>://<?= $_SERVER['HTTP_HOST']; ?><?=WEBROOT; ?>admin/comptes/list/supp_profile_img/" data="<?= $c['id']; ?>">Supprimer la photo de profil</button>
          <?php } else { ?>
               <button type="button" class="btn btn-warning" disabled>Supprimer la photo de profil</button>
          <?php } ?>
          <?php if ($c['is_ban'] == 0 && $c['can_ban'] == true){ ?>
                <button type="button" class="btn btn-danger first_ban_button" data-dismiss="modal" data="<?= $c['id']; ?>">Bannir</button>
          <?php } else if ($c['is_ban'] == 0 && $c['can_ban'] != true) { ?>
                <button type="button" class="btn btn-danger" disabled="disabled">Bannir</button>
          <?php } ?>
        </div>
