  ...
  if (isset($param[1]) && isset($param[2]) && isset($param[3]) && $param[1] == "a"){
    ...
    if ($param[2] == "a_r" && intval($param[3]) && !empty($_POST['content'])){
      ...
      if ($ticketfornotif['pseudo'] != $_SESSION['pseudo']){
        $controleur_def->notify('Une nouvelle réponse sur votre ticket.', $ticketfornotif['pseudo'], 2, "Activité sur votre ticket", LINK . "support/");    
      }else if ($_SESSION['user']->getLevel() < 2) {
        $controleur_def->notify('Une nouvelle réponse sur le ticket de '. $_SESSION['pseudo'], "admin", 2, "Activité sur un Ticket", LINK . "support/");
      }
      exit("Success");
    }else if ($param[2] == "a_t" && !empty($_POST['content']) && !empty($_POST['title'])){
      $content = $_POST['content'];
      $new = createTicket($controleur_def->bddConnexion(), $_SESSION['user']->getId(), $_POST['title'], $content, $_SESSION['user']->getRole());
      $controleur_def->notify('Un ticket a été créé par l\'utilisateur '. $_SESSION['pseudo'], "admin", 2, "Nouveau Ticket", LINK . "support/");
      exit("Success");
    }else {
      exit('Error (4)');
    }
