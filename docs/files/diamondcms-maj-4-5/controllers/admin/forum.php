...
}else if (isset($param[2]) && !empty($param[2]) && $param[2] == "moove" && isset($_POST['id']) && isset($param[3])){
    define('FORCE_INLINE_ERR', true);
    if (simplifySQL\update($controleur_def->bddConnexion(), "d_forum_sous_cat", array(array("id_cat", "=", $_POST['id'])), array(array("id", "=", $param[3]))) == true){
        die('Success');
    }
    die('Error');
}
...
if (isset($_POST) && !empty($_POST)){
    ...
    if (isset($_POST['new_cat']) && !empty($_POST['new_cat'])){
        if (simplifySQL\insert($controleur_def->bddConnexion(), "d_forum_cat", array("titre"), array(clearString($_POST['new_cat']))) != true){
        ...
        if (!empty(simplifySQL\select($controleur_def->bddConnexion(), true, "d_forum_sous_cat", "*", array(array('titre', "=", $_POST['new_scat']))))){
            ...
            if (simplifySQL\insert($controleur_def->bddConnexion(), "d_forum_sous_cat", array("titre", "id_cat"), array(clearString($_POST['new_scat']), $_POST['cat_id'])) != true){
                $controleur_def->addError("342c");
            }
        }
...
foreach($cats as $k => $c){
    $cats[$k]['scats'] = simplifySQL\select($controleur_def->bddConnexion(), false, "d_forum_sous_cat", "*", array(array("id_cat", "=", $cats[$k]['id'])));
    $cats[$k]['nb'] = 0;
    foreach ($cats[$k]['scats'] as &$sc){
        $cats[$k]['nb'] += sizeof(simplifySQL\select($controleur_def->bddConnexion(), false, "d_forum", "id_scat", array(array("id_scat", "=", $sc['id']))));
    } 
