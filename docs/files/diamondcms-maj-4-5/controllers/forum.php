...
//Si on passe en mode résolution
  ...
  header('Location: '. LINK . 'forum');
} else if ($param[1] == "moove" && isset($param[2]) && isset($_SESSION['user']) && isset($_POST['scat'])){
    //On commence par rechercher le sujet qu'on cherche à déplacer
    $post = simplifySQL\select($controleur_def->bddConnexion(), true, "d_forum", "*", array(array("id", "=", intval($param[2]))));
    if ($post == true || is_array($post) || !empty($post)){
      if ($_SESSION['user']->isAdmin()){
        //On retire un sujet de l'ancienne catégorie
        $old_scat = simplifySQL\select($controleur_def->bddConnexion(), true, "d_forum_sous_cat", "*", array(array("id", "=", $post["id_scat"])));
        if (empty($old_scat)){
          die('Error (1)');
        }
        if (!simplifySQL\update($controleur_def->bddConnexion(), "d_forum_sous_cat", array(array("nb_sujets", "=", $old_scat['nb_sujets']-1)), array(array("id", "=", intval($post["id_scat"]))))){
          die('Error (2)');
        }

        $new_scat = simplifySQL\select($controleur_def->bddConnexion(), true, "d_forum_sous_cat", "*", array(array("id", "=", $_POST['scat'])));
        if (empty($new_scat)){
          die('Error (3)');
        }
        if (!simplifySQL\update($controleur_def->bddConnexion(), "d_forum_sous_cat", array(array("nb_sujets", "=", $new_scat['nb_sujets']+1)), array(array("id", "=", intval($_POST['scat']))))){
          die('Error (4)');
        }

        if (simplifySQL\update($controleur_def->bddConnexion(), "d_forum", array(array("id_scat", "=", $_POST['scat'])), array(array("id", "=", intval($param[2]))))){
          die('Success');
        }

        die('Error (5)');
      }
      die ('Error: You aren\'t an admin');
    }
