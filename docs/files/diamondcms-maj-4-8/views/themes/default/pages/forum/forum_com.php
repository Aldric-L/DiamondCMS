...
<div id="fh5co-page-title" style="background-image: url(<?= LINK; ?>views/uploads/img/<?php echo $Serveur_Config['bg']; ?>)">
  ...
  <div class="text">
    <h1>Forum -> <a href="<?php echo LINK . 'forum/'; ?>"><?php echo $cat['titre']; ?></a> -> <a href="<?php echo LINK . 'forum/' . str_replace(' ', '-', $sous_cat[0]['titre']) . '/';?>"><?php echo $sous_cat[0]['titre']; ?> </a>-> <?php echo $post['titre_post']; ?></h1>
  </div>
</div>
<div class="content-container-forum">
  <?php if ($post['user'] != "Utilisateur inconnu"){ ?>
    <div id="f_cat"><h3 class="green"><?php echo $post['titre_post'] . ' par <a style="text-decoration: underline;" class="green" href="' . LINK . 'compte/' .$post['user'] . '">' . $post['user'] .  '</a> le '. $post['date_post']; ?></h3></div>
  <?php }else { ?>
    <div id="f_cat"><h3 class="green"><?php echo $post['titre_post'] . ' par ' . $controleur_def->echoRoleName($controleur_def->bddConnexion(), $post['user']) . $post['user'] .  ' le '. $post['date_post']; ?></h3></div>
  <?php } ?>
  ...
  <?php $i =1; foreach ($coms as $key => $com) {?>
    <?php if ($com['user'] != "Utilisateur inconnu"){ ?>
      <div id="f_com" class="<?php echo $i; ?>"><h3 class="green">En réponse à "<?php echo $post['titre_post'] . '" par <a style="text-decoration: none;" class="green" href="' . LINK . 'compte/' . $com['user'] . '">'  . $com['role'] . '<span style="text-decoration: underline;">' . $com['user'] . '</span>' . '</a> le '. $com['date_com']; ?></h3></div>
    <?php } else { ?>
      <div id="f_com" class="<?php echo $i; ?>"><h3 class="green">En réponse à "<?php echo $post['titre_post'] . '" par ' . $com['role'] . $com['user'] . ' le '. $com['date_com']; ?></h3></div>
    <?php } ?>
  ...
  <?php } $i++; } if ($i >= 11){ ?>
  <button class="hider btn pull-right btn-green sub">Voir les réponses cachées...</button><br /><br/>
  <?php } ?>
  ...
  <?php }else if ($resolu) {
    ...
    echo '<p class="text-center"><em>Pour répondre  <a class="green" href="'. LINK . 'connexion"> <i class="fa fa-key" aria-hidden="true"></i> Connectez-vous !</a></em><p>';
  } ?>
</div>