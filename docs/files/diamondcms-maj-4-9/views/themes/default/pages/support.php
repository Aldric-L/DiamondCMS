      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form method="post" id="newticket_form">
            ...
            <input type="hidden" class="hidden_url_nt" id="hidden_url_nt" value="<?= LINK; ?>support/a/a_t/"/>
            <br />
            <h4 class="text-center text-danger">Attention, une fois votre ticket envoyé,<br/> il sera impossible à supprimer.</h4><br/>
            <button type="submit" class="btn btn-success align-right center-block green">Créer le ticket</button>
          </form>
        </div>
...
<div class="content-container container">
  ...
  <table class="table table-striped">
    ...
    <tbody>
      <?php foreach ($tickets as $key => $ticket) {
        ...
        echo '<td><img width=26 height=26 src="' . LINK . 'getprofileimg/'. $tickets[$key]['pseudo'] . '/26"></td><td> ' . $tickets[$key]['pseudo'] . "</td>";
  ...
  <div id="modal_ticket_<?php echo $tickets[$key]['id']; ?>" class="modal fade">
      <div class="modal-dialog">
          <div class="modal-content">
              ...
              <div class="modal-body">
                  <h3><?php echo $tickets[$key]['titre_ticket'] . ' par ' . '<img width=28 height=28 src="' . LINK . 'getprofileimg/'. $tickets[$key]['pseudo'] . '/26"> ' . $tickets[$key]['pseudo'] . ' le '. $tickets[$key]['date_t']; ?></h3>
                ...
                <div id="r_<?php echo $tickets[$key]['id']; ?>" class="response" <?php if ($tickets[$key]['status'] == 2){?>style="display: none;"<?php } ?>>
                  ...
                  <div class="center" id="rep_div_<?php echo $tickets[$key]['id']; ?>" style="display: none;">
                      ...
                      <input type="hidden" class="hidden_url" id="hidden_url_<?php echo $tickets[$key]['id']; ?>" value="<?= LINK; ?>support/a/a_r/<?php echo $tickets[$key]['id']; ?>"/>
  ...
  //Systeme de fermeture de tickets
    if ((isset($_SESSION['user']) && $_SESSION['user'] && $_SESSION['user']->getLevel() >= 2) || (isset($_SESSION['pseudo']) && $_SESSION['pseudo'] == $tickets[$key]['pseudo'])) {?>
          ...
          $(".d_<?php echo $tickets[$key]['id']; ?>").click(function(){
              $.ajax({
                 url : '<?= LINK; ?>support/a/d_t/<?php echo $tickets[$key]['id']; ?>/',
                 type : 'GET',
    ...
    foreach($tickets[$key]['rep'] as $rep){  
      if ((isset($_SESSION['user']) && $_SESSION['user'] && $_SESSION['user']->getLevel() >= 2) || (isset($_SESSION['pseudo']) && $_SESSION['pseudo'] == $rep['pseudo'])) {?>
            ...
            $("#d_r_<?php echo $rep['id']; ?>").click(function(){
                $.ajax({
                  url : '<?= LINK; ?>support/a/d_r/<?php echo $rep['id']; ?>/',
                  type : 'GET',
                  success: function (data_rep) {
                      if (data_rep != "Success"){
                        alert("Erreur, Code 112, Merci de contacter les administrateurs du site.");
   ...
   if (isset($param[1]) && is_int($param[1]) && sizeof($ntickets)-($param[1]*10) >= 10){
    if (!isset($param[1])){?>
  ...
  <?php }else { ?>
    <span class="text-right bold" style="float: right;"><a class="bold" href="<?= LINK; ?>support/<?php echo $param[1]+1;?>">Page suivante <i class="fa fa-angle-double-right" aria-hidden="true"></i></a></span>
  <?php }}
    ...
    <?php }else if (isset($param[1]) && $param[1] >= 2){?>
      <span class="text-left bold" style="float: left;"><a class="bold" href="<?= LINK; ?>support/<?php echo $param[1]-1;?>"><i class="fa fa-angle-double-left" aria-hidden="true"></i>Page précedente</a></span>
    <?php }
...
<div class="content-container container">
    ...
    <div class="col-sm-3">
      <br />
          <h4 class="text-center">Action(s) :</h4>
            <h5 class="text-center"><button class="btn btn-green addTicket" style="border-color: #197d62;background-color: #197d62;">Créer un nouveau ticket</button></h5>
            <h5 class="text-center"><button class="btn btn-green" style="border-color: #197d62;background-color: #197d62;"><a class="no" style="text-decoration: none;color: white;" href="<?= LINK; ?>forum">Retourner sur le forum...</a></button></h5>
    </div>
  <!-- FIN col-lg-3 -->
    <div class="col-sm-9">
  ...
  <?php }else { ?>
    <span class="text-right bold" style="float: right;"><a class="bold" href="<?= LINK; ?>support/<?php echo $param[1]+1;?>">Page suivante <i class="fa fa-angle-double-right" aria-hidden="true"></i></a></span>
  <?php }}
    if (isset($param[1]) && is_int($param[1]) && $param[1] == 1){?>
      <span class="text-left bold" style="float: left;"><a class="bold" href="<?= LINK; ?>support/"><i class="fa fa-angle-double-left" aria-hidden="true"></i> Page précedente</a></span>
    <?php }else if (isset($param[1]) && $param[1] >= 2){?>
      <span class="text-left bold" style="float: left;"><a class="bold" href="<?= LINK; ?>support/<?php echo $param[1]-1;?>"><i class="fa fa-angle-double-left" aria-hidden="true"></i> Page précedente</a></span>
