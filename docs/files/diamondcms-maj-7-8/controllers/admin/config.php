...
if (isset($param[2]) && !empty($param[2]) && isset($param[3]) && !empty($param[3]) && $param[2] == "write" ){
    ...
    if ($param[3] == "mainconf"){
        ...
        if (isset($_POST['Serveur_name']) &&
            ...
            isset($_POST['favicon']) &&
            isset($_POST['logo'])&&
            isset($_POST['tinymce_en'])&&
            isset($_POST['tinymce_key']) ){
            //Ecriture dans le fichier ini
                ...
                $temp_conf_mce = parse_ini_file(ROOT . "config/tinymce.ini", true);
                if ($_POST['tinymce_en'] == "true"){
                    $temp_conf_mce['editor']['enable'] = 1;
                }else {
                    $temp_conf_mce['editor']['enable'] = 0;
                }
                if (!empty($_POST['tinymce_key'])){
                    $temp_conf_mce['editor']['key'] = $_POST['tinymce_key'];
                }
                $ini = new ini (ROOT . "config/tinymce.ini", 'Configuration DiamondCMS');
                //On lui passe l'array modifié
                $ini->ajouter_array($temp_conf_mce);
                //On écrit en lui demmandant de conserver les groupes
                $ini->ecrire(true);
            //FIN Encriture ini
            die('Success');
