...
if (isset($param[2]) && !empty($param[2]) && $param[2] == "xhr" && isset($param[3]) && !empty($param[3])){
    ...
    if ($param[3] == "enable"){
        ...
        if ($Serveur_Config['en_boutique']){
            ...
            $temp_conf['boutique_ext']['en_boutique_externe'] = 0;
        }
    ...
    //Si on modifie les réglages généraux
    }else if ($param[3] == "saveconf" && !empty($_POST) && isset($_POST['money_sym']) && isset($_POST['money_name']) && isset($_POST['money'])&& isset($_POST['link_boutique_externe'])&& isset($_POST['en_boutique_externe'])){
        //Ecriture dans le fichier ini
        //Copie du fichier dans un array temporaire
        $temp_conf = $Serveur_Config;
        ...
        $temp_conf['boutique_ext']['link_boutique_externe'] = $_POST['link_boutique_externe'];
        if ($_POST['en_boutique_externe'] == "true"){
            $temp_conf['boutique_ext']['en_boutique_externe'] = 1;
        }else {
            $temp_conf['boutique_ext']['en_boutique_externe'] = 0;
        }
        //On appel la class ini pour réecrire le fichier
