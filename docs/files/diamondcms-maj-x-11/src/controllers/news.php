...
if (isset($param[1]) && !empty($param[1])){
    $news = simplifySQL\select($controleur_def->bddConnexion(), true, "d_news", array("id", "name", "content_new", array("date", "%d/%m/%Y à %h:%i:%s", "date"), "img", "user"), array("id", "=", $param[1]), "date", true, false);
