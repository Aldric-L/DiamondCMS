...
function list_dir($name) {
  ...
  if ($dir = opendir($name)) {
    while($file = readdir($dir)) {
      if(is_dir($name . $file) && !in_array($file, array(".","..")) && substr(sprintf('%o', fileperms($name . $file)), -4) == "0777") {
        list_dir($name  . $file . '/');
      }else if(!in_array($file, array(".","..")) && substr(sprintf('%o', fileperms($name . $file)), -4) != "0777") {
		    array_push($errors_chmod, array($name . $file, substr(sprintf('%o', fileperms($name . $file)), -4), "0777"));
	    }
    }
    closedir($dir);
  }
...
foreach ($files as $f){
    if (substr(sprintf('%o', fileperms($f)), -4) != "0777"){
        chmod($f, 0777);
        if (substr(sprintf('%o', fileperms($f)), -4) != "0777"){
          array_push($errors_chmod, array($f, substr(sprintf('%o', fileperms($f)), -4), "0777"));
        }
    }else {
		$search = list_dir($f);
      if (!empty($search)){
