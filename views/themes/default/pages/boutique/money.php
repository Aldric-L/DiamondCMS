...
<div id="fh5co-page-title" style="background-image: url(<?php echo $Serveur_Config['protocol']; ?>://<?= $_SERVER['HTTP_HOST']; ?><?=WEBROOT; ?>views/uploads/img/<?php echo $Serveur_Config['bg']; ?>)">
  ...
  <div class="text">
    <h1><a class="no" href="<?php echo $Serveur_Config['protocol']; ?>://<?php echo $_SERVER['HTTP_HOST'] . WEBROOT . 'boutique/' ?>">Boutique </a>-> Achat de <?= $Serveur_Config['Serveur_money']; ?>s</h1>
  </div>
</div>
<style>
a.no {
...
<div class="container">
    ...
    <?php if ($boutique_config['PayPal']['en_paypal'] == "true"){ ?>
        ...
              payment: function(data, actions) {
                var CREATE_URL = '<?php echo $Serveur_Config['protocol']; ?>://<?php echo $_SERVER['HTTP_HOST'] . WEBROOT . 'boutique/getmoney/createpaypal/' . $po['id']; ?>';
        ...
              onAuthorize: function(data, actions) {
                  var EXECUTE_URL = '<?php echo $Serveur_Config['protocol']; ?>://<?php echo $_SERVER['HTTP_HOST'] . WEBROOT . 'boutique/getmoney/buypaypal/' . $po['id']; ?>';
                  var data = {
                ...
                  return paypal.request.post(EXECUTE_URL, data)
                        .then(function (data) { 
                                    console.log(data);
                        if (data.success) { 
                                window.location.replace("<?php echo $Serveur_Config['protocol'] . '://' . $_SERVER['HTTP_HOST'] . WEBROOT . 'boutique/getmoney/successpaypal/'; ?>");
                        } else {
