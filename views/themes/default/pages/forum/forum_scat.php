  ...
  echo '<h4 class="text-right">Pages : ';
    for ($i = 1; $i <= $pages; $i++){
      if ($i == $cur_page){
        ...
        echo '<a href="'. $Serveur_Config['protocol'] .'://'. $_SERVER['HTTP_HOST'] . WEBROOT . 'forum/' . str_replace(' ', '-', $sous_cat['titre']) . '/' . ($i-1) . '"><button class="btn btn-success">' . $i .'</button></a>  ';
      }
    }
    echo '</h4></div>';
 } ?>
...
if (isset($_SESSION['pseudo']) && !empty($_SESSION['pseudo'])){?>
  ...
  <br /></br />
<?php }else {?>
<p class="text-center"><em>Une question ? Pour créer un sujet <a href="<?php echo $Serveur_Config['protocol'] . '://'. $_SERVER['HTTP_HOST'] . WEBROOT . 'connexion'; ?>"> <i class="fa fa-key" aria-hidden="true"></i> Connectez-vous !</a></em><p>
<br /><br />
<?php } ?>
